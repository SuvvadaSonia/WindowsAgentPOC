...
class RemoteAgent:
    ...
    def connect_to_remote_host(self,client):  # It establishes connection with remote device 
        client.set_missing_host_key_policy(paramiko.AutoAddPolicy())
        client.connect(hostname= self.hostname, username= self.username, password= self.password)
        
        
    def execute_commands(self,client):   # It executes commands in remote device and prints response on local device
    ...
    def close_connection(self,client):  # It closes the established connection with remote device 
        client.close()
